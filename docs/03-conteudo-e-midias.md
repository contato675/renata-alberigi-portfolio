...
## O que já existe
Biografia aprovada em português e tradução inglesa com revisão humana pendente em `site/content/artist.json`; nome Renata Alberigi, localização resumida e e-mail informado na conversa. `works.json` começa vazio. Retrato, vídeo, PDF e fotos de ateliê são null ou listas vazias. Não há acervo fictício.
...
Cada obra tem um registro em `site/content/works.json`, validado pelo contrato de `schemas/work.schema.json`. Campos: id, título, ano/intervalo, técnica, dimensões (texto), descrição, categoria, imagens e índice da capa. Título/técnica/descrição/categoria e alt/legenda usam objetos `{ "en": "…", "pt-BR": "…" }`, sem fallback silencioso. Título original pode permanecer em `originalTitle`. O schema é a autoridade exata dos campos. A posição na lista determina a ordem editorial. Cada imagem tem path relativo, alt, width e height obrigatórios; legenda é opcional. Registro só pode ficar published quando a artista aprovar as fotografias e a ficha.
...
- Vídeo: provedor YouTube/Vimeo e identificador permitido, título e transcrição nos dois idiomas, capa com metadados; sem arquivo pesado neste Git.
...
## Separação pública
O build copia somente mídia referenciada por obras com status `published`, retrato/processo/vídeo aprovados e PDF indicado. Rascunhos e arquivos soltos não entram em HTML, Markdown, JSON, sitemap, llms ou artefato. O build remove saídas antigas que ele próprio gerou e recusa apagar arquivos desconhecidos. Publicação depende também das flags de aprovação; status `published` sozinho não ativa o Pages.
