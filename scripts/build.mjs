...
export async function buildSite(root,{output=path.join(root,'dist'),data=null,release=false}={}) {
  ...
  for (const rel of ['assets/css/tokens.css','assets/css/scaffold.css','assets/css/curriculum.css','assets/css/gallery.css','assets/js/gallery.js','assets/css/navigation.css','assets/js/navigation.js','assets/js/language-picker.js','assets/js/locale-navigation.js','assets/css/brand.css','assets/js/brand-carousel.js']) files.set(rel,await readFile(path.join(root,'site',rel)));
  ...
  for (const rel of new Set([...imagePaths(images),data.artist.pdf,data.artist.pdfPt,data.artist.pdfFr,data.artist.pdfEs].filter(Boolean))) files.set(rel,await readFile(containedPath(path.join(root,'site'),rel)));
