...
export function pageMarkdown(data,locale,work=null) {
  ...
  if (!work) {
    ...
    text += line(locale === 'en' ? 'Artist' : 'Artista',artist.name) + line(work.yearIsCollectionPeriod?u.collectionPeriod:u.year,work.year) + line(u.technique,l(work.technique,locale)) + (work.dimensions?line(u.dimensions,work.dimensions):'');
