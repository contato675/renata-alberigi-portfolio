...
function localeNav(data, locale, work, notFound = false) {
  return `<nav class="locale-nav" aria-label="${e(data.dictionaries[locale].language)}">${LOCALES.map((other) => `<a class="control" data-locale-link href="${e(localUrl(data.site,pagePath(other,work,notFound)))}" lang="${other}" hreflang="${other}"${locale === other ? ' aria-current="page"' : ''}>${e(data.dictionaries[other].localeName)}</a>`).join('')}</nav>`;
}
...
export function renderPage(data,template,locale,{work=null,release=false,notFound=false}={}) {
  ...
  const route = pagePath(locale,work,notFound), canonical = absoluteUrl(site,route);
  ...
  const description = notFound ? u.notFoundText : work ? (work.description ? l(work.description,locale) : `${l(work.title,locale)} · ${work.year} · ${l(work.technique,locale)} · ${work.dimensions||''}`) : l(artist.intro,locale);
  const header = renderNavigation(data,locale,{homeUrl,localeMarkup:localeNav(data,locale,work,notFound),hasDigital:publicWorks(data).some(w=>w.collection==='digital'),hasFilm:!!artist.featuredVideo||preview,hasBrand:publicWorks(data).some(w=>w.collection==='brand-design')});
