...
import {renderCurriculum} from './cv-render.mjs';
import {escapeHtml as e,allVideos,pdfPath} from './content.mjs';
import {localized as l, localePath, LOCALES, LOCALE_CONFIG} from './i18n.mjs';
...
export const pagePath = (locale, work = null, notFound = false) => localePath(locale) + (notFound ? '404.html' : work ? `works/${work.id}/` : '');
...
function localeNav(data, locale, work, notFound = false, curriculum = false) {
  return `<nav class="locale-nav" aria-label="${e(data.dictionaries[locale].language)}">${LOCALES.map((other) => `<a class="control" data-locale-link href="${e(localUrl(data.site,(curriculum ? cvPath(other) : pagePath(other,work,notFound))))}" lang="${other}" hreflang="${other}"${locale === other ? ' aria-current="page"' : ''}>${e(data.dictionaries[other].localeName)}</a>`).join('')}</nav>`;
}
...
function card(work, data, locale) {
  ...
  return `<li class="project"><a href="${e(localUrl(site,pagePath(locale,work)))}" data-open-project="${e(work.id)}" aria-labelledby="card-${e(work.id)}"><figure>${picture(work.images[work.cover],locale,site,false,'','(min-width: 1488px) 448px, (min-width: 1024px) 31vw, (min-width: 768px) 47vw, 94vw')}<figcaption class="caption"><h3 id="card-${e(work.id)}" translate="no" class="notranslate">${e(l(work.title,locale))}</h3><p>${work.yearIsCollectionPeriod?e(u.collectionPeriod)+': ':''}${e(work.year)} · ${e(l(work.technique,locale))}</p>${work.dimensions?`<p class="work-dimensions">${e(work.dimensions)}</p>`:''}${work.images.length > 1 ? `<span class="project-number">${work.images.length} ${e(u.images)}</span>` : ''}</figcaption></figure></a></li>`;
}
function dialog(work, data, locale) {
  const {site} = data, u = data.dictionaries[locale];
  return `<dialog class="viewer" data-project="${e(work.id)}" data-cover="${work.cover}" aria-labelledby="viewer-${e(work.id)}"><div class="viewer-head"><h2 id="viewer-${e(work.id)}" translate="no" class="notranslate">${e(l(work.title,locale))}</h2><button type="button" data-close>${e(u.close)}</button></div><div class="viewer-layout"><div class="viewer-media"><div class="image-rail" role="region" aria-label="${e(l(work.title,locale))}">${work.images.map((im, i) => `<figure aria-label="${e(u.image)} ${i+1} ${e(u.of)} ${work.images.length}">${picture(im,locale,site)}${im.caption ? `<figcaption>${e(l(im.caption,locale))}</figcaption>` : ''}</figure>`).join('')}</div><div class="viewer-controls"${work.images.length === 1 ? ' hidden' : ''}><button type="button" data-prev><span aria-hidden="true">←</span><span class="sr-only">${e(u.previous)}</span></button><output aria-live="polite" aria-atomic="true" data-of="${e(u.of)}">${work.cover+1} ${e(u.of)} ${work.images.length}</output><button type="button" data-next><span aria-hidden="true">→</span><span class="sr-only">${e(u.next)}</span></button></div></div><aside>${facts(work,locale,u)}<a class="control" href="${e(localUrl(site,pagePath(locale,work)))}">${e(u.openFull)}</a><p class="meta">${e(u.viewLanguage)}</p>${localeNav(data,locale,work)}</aside></div></dialog>`;
}
function film(data, locale, preview) {
  ...
  return `<section id="film" aria-labelledby="film-title"><div class="section-heading"><p class="eyebrow">02 / ${e(u.video)}</p><h2 id="film-title">${e(u.filmTitle)}</h2></div>${videos.length?`<div class="film-grid grid">${cards}</div>`:`<div class="placeholder video-slot"><p>${e(u.filmPending)}</p></div>`}</section>`;
}
...
function home(data,locale,preview) {
 ...
 <section id="about" class="grid" aria-labelledby="about-title"><div class="section-title section-heading"><p class="eyebrow">${data.works.some(w=>w.collection==='brand-design')?'05':'04'} / ${e(u.bioLabel)}</p><h2 id="about-title">${e(artist.name)}</h2></div><div class="section-content bio">${paragraphs(l(artist.bio,locale))}${data.curriculum?`<a class="control cv-biography-link" data-cv-link href="${e(localUrl(site,cvPath(locale)))}">${e(u.cvLink)} <span aria-hidden="true">↗</span></a>`:''}</div>${guides(preview)}</section>
...
function workPage(work,data,locale) {
  ...
  return `<article itemscope itemtype="https://schema.org/VisualArtwork" itemid="${e(absoluteUrl(site,pagePath(locale,work)))}"><div class="work-heading"><a class="control" href="${e(localUrl(site,pagePath(locale)))}#${work.collection==='digital'?'digital':'works'}">${e(u.back)}</a><h1 itemprop="name" translate="no" class="notranslate">${e(l(work.title,locale))}</h1><p itemprop="creator" itemscope itemtype="https://schema.org/Person"><span itemprop="name">${e(artist.name)}</span></p></div><div class="grid"><div class="work-images">${work.images.map((im,i) => `<figure>${picture(im,locale,site,i === 0,'itemprop="image"')}${im.caption ? `<figcaption>${e(l(im.caption,locale))}</figcaption>` : ''}</figure>`).join('')}</div><aside class="work-info">${facts(work,locale,u)}</aside></div></article>`;
}
export function renderPage(data,template,locale,{work=null,release=false,notFound=false,curriculum=false}={}) {
  const {artist,site} = data, u = data.dictionaries[locale], preview = !release;
  const route = curriculum ? cvPath(locale) : pagePath(locale,work,notFound), canonical = absoluteUrl(site,route);
  const homeUrl = localUrl(site,pagePath(locale));
  const title = curriculum ? `${u.cvTitle} — ${artist.name}` : notFound ? `${u.notFound} — ${artist.name}` : `${work ? l(work.title,locale) : artist.name} — ${l(artist.role,locale)}`;
  const description = curriculum ? u.cvIntro : notFound ? u.notFoundText : work ? (work.description ? l(work.description,locale) : `${l(work.title,locale)} · ${work.year} · ${l(work.technique,locale)} · ${work.dimensions||''}`) : l(artist.intro,locale);
  const header = renderNavigation(data,locale,{homeUrl,localeMarkup:localeNav(data,locale,work,notFound,curriculum),curriculum,cvUrl:data.curriculum?localUrl(site,cvPath(locale)):null,hasDigital:publicWorks(data).some(w=>w.collection==='digital'),hasFilm:!!artist.featuredVideo||preview,hasBrand:publicWorks(data).some(w=>w.collection==='brand-design')});
  const footer = curriculum ? `<footer id="cv-footer"><div class="wrap"><div class="footer-links"><a class="control" href="${e(homeUrl)}">${e(u.cvBack)}</a><a class="control" href="${e(localUrl(site,route+'index.md'))}">${e(u.machine)}</a></div><p class="meta">${e(u.rights)}</p></div></footer>` : `<footer id="contact"><div class="wrap"><p class="eyebrow">${e(u.contact)}</p><p>${e(u.contactIntro)}</p><div class="footer-links"><a class="control" href="mailto:${e(artist.email)}">${e(artist.email)}</a><a class="control" href="${e(artist.instagram)}" target="_blank" rel="noopener noreferrer" aria-label="${e(u.instagramProfile)}">${e(u.instagram)} <span aria-hidden="true">↗</span></a>${pdfPath(artist,locale) ? `<a class="control" href="${e(localUrl(site,pdfPath(artist,locale)))}" type="application/pdf">${e(u.pdf)}</a>` : ''}${notFound?'':`<a class="control" href="${e(localUrl(site,route+'index.md'))}">${e(u.machine)}</a>`}</div><p class="meta">${e(u.rights)}</p></div></footer>`;
  const alternates = LOCALES.map((lang) => `<link rel="alternate" hreflang="${lang}" href="${e(absoluteUrl(site,(curriculum?cvPath(lang):pagePath(lang,work,notFound))))}">`).join('\n') + `\n<link rel="alternate" hreflang="x-default" href="${e(absoluteUrl(site,(curriculum?cvPath('en'):pagePath('en',work,notFound))))}">`;
  const values = {LANG:locale,ROBOTS:release && !notFound ? 'index,follow,max-image-preview:large' : 'noindex,follow',TITLE:e(title),DESCRIPTION:e(description),CANONICAL:e(canonical),OGIMAGE:!curriculum&&publicWorks(data).length?`<meta property="og:image" content="${e(absoluteUrl(site,(work||publicWorks(data)[0]).images[(work||publicWorks(data)[0]).cover].path))}">`:'',OGLOCALE:LOCALE_CONFIG[locale].og,ALTERNATES:alternates,BASE:e(site.basePath),LLMS:e(absoluteUrl(site,'llms.txt')),MARKDOWNLINK:notFound?'':`<link rel="alternate" type="text/markdown" href="${e(absoluteUrl(site,route+'index.md'))}" title="${e(u.machine)}">`,DATATITLE:e(curriculum?u.cvDataTitle:u.structuredDataTitle),DATA:e(absoluteUrl(site,curriculum?route+'curriculum.json':localePath(locale)+'portfolio.json')),PAGESTYLES:curriculum?`<link rel="stylesheet" href="${e(localUrl(site,'assets/css/curriculum.css'))}?v=20260908-cv1">`:'',STAGE:preview ? 'review' : 'published',SKIP:e(u.skip),HEADER:header,NOTICE:'',MAIN:curriculum ? renderCurriculum(data,locale) : notFound ? `<section><h1>${e(u.notFound)}</h1><p>${e(u.notFoundText)}</p><a class="control" href="${e(homeUrl)}">${e(u.home)}</a></section>` : (work ? workPage(work,data,locale) : home(data,locale,preview)),FOOTER:footer,DIALOGS:(notFound||curriculum)?'':work?(work.collection==='brand-design'?dialog(work,data,locale):''):publicWorks(data).map((w)=>dialog(w,data,locale)).join('\n')};
