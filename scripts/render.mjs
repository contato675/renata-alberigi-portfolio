...
function facts(work, locale, u) {
  ...
  return `<dl><dt>${e(yearLabel)}</dt><dd${/^\d{4}$/.test(work.year)?' itemprop="dateCreated"':''}>${e(work.year)}</dd><dt>${e(u.technique)}</dt><dd itemprop="artMedium">${e(l(work.technique,locale))}</dd>${work.dimensions?`<dt>${e(u.dimensions)}</dt><dd>${e(work.dimensions)}</dd>`:''}</dl>${work.titleStatus==='catalogue-label'?`<p class="meta">${e(u.catalogueLabel)}</p>`:''}${work.description?paragraphs(l(work.description,locale)):''}`;
}
function card(work, data, locale) {
  const {site} = data, u = data.dictionaries[locale];
  return `<li class="project"><a href="${e(localUrl(site,pagePath(locale,work)))}" data-open-project="${e(work.id)}" aria-labelledby="card-${e(work.id)}"><figure>${picture(work.images[work.cover],locale,site,false,'','(min-width: 1488px) 448px, (min-width: 1024px) 31vw, (min-width: 768px) 47vw, 94vw')}<figcaption class="caption"><h3 id="card-${e(work.id)}">${e(l(work.title,locale))}</h3><p>${work.yearIsCollectionPeriod?e(u.collectionPeriod)+': ':''}${e(work.year)} · ${e(l(work.technique,locale))}</p>${work.images.length > 1 ? `<span class="project-number">${work.images.length} ${e(u.images)}</span>` : ''}</figcaption></figure></a></li>`;
}
...
export function renderPage(data,template,locale,{work=null,release=false,notFound=false}={}) {
  ...
  const header = `<header><div class="wrap header-inner"><a class="brand control" href="${e(homeUrl)}">${e(artist.name)}</a><nav class="main-nav" aria-label="${e(u.mainNav)}"><a class="control" href="${e(homeUrl)}#works">${e(u.paintingNav)}</a>${publicWorks(data).some(w=>w.collection==='digital')?`<a class="control" href="${e(homeUrl)}#digital">${e(u.digital)}</a>`:''}${artist.featuredVideo || preview ? `<a class="control" href="${e(homeUrl)}#film">${e(u.video)}</a>` : ''}<a class="control" href="${e(homeUrl)}#about">${e(u.about)}</a><a class="control" href="#contact">${e(u.contact)}</a></nav>${localeNav(data,locale,work)}</div></header>`;
  const footer = `<footer id="contact"><div class="wrap"><p class="eyebrow">${e(u.contact)}</p><p>${e(u.contactIntro)}</p><div class="footer-links"><a class="control" href="mailto:${e(artist.email)}">${e(artist.email)}</a>${pdfPath(artist,locale) ? `<a class="control" href="${e(localUrl(site,pdfPath(artist,locale)))}" type="application/pdf">${e(u.pdf)}</a>` : ''}<a class="control" href="${e(localUrl(site,route+'index.md'))}">${e(u.machine)}</a></div><p class="meta">${e(u.rights)}</p>${preview ? `<button type="button" class="grid-toggle" data-grid-toggle aria-pressed="false" hidden>${e(u.grid)}</button>` : ''}</div></footer>`;
