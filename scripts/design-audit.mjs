...
try {
  ...
  const report={date:new Date().toISOString(),browser:browserVersion.product,platform:process.platform,policy:'Apple-like + Müller-Brockmann; no certification by Apple',status:failures?'FAIL':'PASS_WITH_EDITORIAL_PENDING',scope:{actual:'Unpublished portfolio without supplied artwork photographs',fixture:'Isolated neutral calibration media, not real artwork; never part of dist/',widths,locales:['en','pt-BR']},results,checks,environmentResources,environmentNote:environmentResources.length?'Observed OS antivirus injection, not app code. Security software was not disabled; external hosts are reported without query strings.':null,pending:['Visual judgement using actual artwork, portrait and final video','Human review of the English biography','VoiceOver/TalkBack and real iOS/Android touch gestures','Real browser 200% text zoom and media fidelity/colour','Field performance measurements and live origin-root robots verification'],failures};
