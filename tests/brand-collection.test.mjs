...
test('The new single horizontal row comes immediately after digital paintings',()=>{
 for(const locale of ['en','pt-BR']){const html=out.get(pagePath(locale,null)+'index.html');const start=html.indexOf('<section id="brand-design"'),digital=html.indexOf('<section id="digital"'),about=html.indexOf('<section id="about"');assert.ok(digital<start&&start<about);assert.equal((html.match(/class="brand-rail"/g)||[]).length,1);const section=html.slice(start,about);assert.equal((section.match(/data-start=/g)||[]).length,66);assert.doesNotMatch(section,/class="gallery grid"/);assert.ok(section.includes(pagePath(locale,work)));}
});
test('Dedicated collection pages contain all 66 designs and a single complete viewer',()=>{
 for(const locale of ['en','pt-BR']){const html=out.get(pagePath(locale,work)+'index.html');assert.ok(html.includes('https://schema.org/CreativeWork'));assert.equal((html.match(/<figure id="brand-image-/g)||[]).length,66);assert.equal((html.match(/data-project="ruadoflow-brand-design/g)||[]).length,1);assert.ok(html.includes(work.images.at(-1).path));assert.ok(out.get(pagePath(locale,work)+'index.md').includes(work.images.at(-1).path));}
});
test('Design collection is distinguished from painting in structured data',()=>{
 for(const locale of ['en','pt-BR']){const json=JSON.parse(out.get((locale==='en'?'':'pt-br/')+'portfolio.json'));const w=json['@graph'].find(w=>w.url?.includes(work.id));assert.equal(w['@type'],'CreativeWork');assert.equal(w.artMedium,undefined);assert.equal(w.artform,undefined);assert.equal(w.dateCreated,undefined);assert.equal(w.temporalCoverage,'2024/2025');assert.equal(w.genre,work.technique[locale]);assert.equal(w.image.length,66);}
});
test('Machine-readable indexes include the collection with its own heading',()=>{assert.ok(out.get('llms.txt').includes('RUADOFLOW — Brand Design Collection'));assert.ok(out.get('llms-full.txt').includes(work.id));for(const locale of ['en','pt-BR'])assert.ok(out.get(pagePath(locale,null)+'index.md').includes(work.id));});
